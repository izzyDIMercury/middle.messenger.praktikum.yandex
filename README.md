...
### ! Удаление чата:
После удаления чата выполняется переключение на другой чат, и он не работает (ошибку пока не смог устранить), чтобы заработало, нужно перезагрузить страницу

### ! Автотесты не проходят:
Изначально была проблема с роутингом в express при отдаче статики, страницы кроме стартовой не работали и автотест на этом моменте также не проходил. Попробовал несколько вариантов решения проблемы, все они работают и локально express работает, страницы загружаются. Но автотест по прежнему выдает на этом месте ошибку, якобы страница не найдена, пока как решить проблему не знаю; 

## Версия NodeJS:

v20.11.1

### Аккаунты (логин, пароль, id):
